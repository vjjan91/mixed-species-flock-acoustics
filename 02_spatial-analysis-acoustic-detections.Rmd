...
# add site name to acoustic data
acoustic_data <- left_join(acoustic_data[,-9], metadata[,c(1,2)])
```
...
metadata <- metadata %>%
  ...
  mutate(site_name_ordered = factor(site_name, 
                 levels = unique(site_name)))

## for the visualization
range_info <- metadata %>%
  group_by(site_name) %>%
  summarise(min_point = min(as.numeric(site_name_ordered)),
            max_point = max(as.numeric(site_name_ordered)))

# create date breaks starting in November 2024
date_breaks <- seq(as.Date("2024-02-01"), 
...
fig_samplingEffort <- metadata %>%
  ...
  ggplot(., aes(y = site_name_ordered)) +  
  geom_segment(aes(x = first_file_date, xend = last_file_date, 
                   yend = site_name_ordered), 
               color = "#d95f02", size = 1) +
 scale_x_date(
    limits = as.Date(c("2024-02-01", "2024-04-01")), 
    breaks = seq(as.Date("2024-02-01"), as.Date("2024-04-01"), by = "month"),
    date_labels = "%b %Y",
...
ggsave(fig_samplingEffort, filename = "figs/sampling_effort.png", width = 14, height = 7, device = png(), units = "in", dpi = 300)
dev.off()
```

![](figs/sampling_effort.png)  
...
recording_days <- metadata %>%
  filter(site_name != "Pudhuthottam") %>%
  rowwise() %>%
  mutate(
    days = list(seq(first_file_date, last_file_date, by = "day"))
  ) %>%
  unnest(days) %>%
  distinct(days, site_name) %>%
  group_by(site_name) %>%
  summarise(
...
fig_totalRecordedDays <- recording_days %>%
  ...
  ggplot(., aes(x = site_name, y = total_recording_days)) +
  geom_bar(stat = "identity", fill = "#d95f02", alpha = 0.9) +
  geom_text(aes(label = total_recording_days), 
            vjust = -0.5, size = 5, family = "Century Gothic") + theme_bw() +
  labs(
    x = "\nSite Name",
...
ggsave(fig_totalRecordedDays, filename = "figs/total_days_audioRecording.png", width = 12, height = 8, device = png(), units = "in", dpi = 300)
dev.off()
```

![](figs/total_days_audioRecording.png)
...
# group detections by day, species and site_name and filter out the above 
...
fig_detections_days <-  detections %>%
  filter(common_name %in% c("Brown-cheeked Fulvetta","Orange Minivet",
                          "Indian White-eye")) %>%
  ...
  scale_x_date(date_labels = "%b %d %Y",
               date_breaks = "3 days") + 
  # faceting by species
  facet_grid(common_name ~ ., scales = "free_y", space = "free_y") +
  labs(title = "Acoustic detections of species across sites and days",
...
ggsave(fig_detections_days, filename = "figs/detections_by_days.png", width = 14, height = 7, device = png(), units = "in", dpi = 300)
dev.off()
```

![](figs/detections_by_days.png)
